...
use crate::akai_apc_mini_mk2::{GRID_MASK, NOTE_ON_STATUS, PULSING_1_2};
...
fn fibble_destinations() -> Result<(), AppError> {
    ...
    let source = Sources
        ...
        .find(|s| {
            s.display_name()
                .map_or(false, |name| name == "APC mini mk2 Control")
        })
...
fn listen_to_color_selection(
    ...
    let callback = enclose!(
        (global_state) move |event_list: &EventList, context: &mut u32| {
